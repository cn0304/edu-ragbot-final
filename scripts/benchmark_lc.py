...
# === A) Backend choice (default to LangChain for this script) ===
BACKEND = os.getenv("RETRIEVER_BACKEND", "langchain")
...
def main() -> None:
    ...
    try:
        ...
        # If current engine doesn't accept retriever_backend, fall back.
        rag = SmartRAGEngine()
        try:
    ...
    # Scalability under workload (match your LlamaIndex script: concurrency=8)
    ...
    root = os.path.dirname(os.path.dirname(__file__))
    out = os.path.join(root, 'metrics', 'benchmark_lc.json')
