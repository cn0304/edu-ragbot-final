...
async def test_query(rag: SmartRAGEngine, query: str, show_full: bool = False):
    ...
    # Show sources (run same retrieval for this query)
    sources = rag.get_sources(query=query)
    print(f"\n📚 SOURCES:")
    ...
    for i, source in enumerate(sources, 1):
        uni = source.get("university") or source.get("university_short") or "Unknown university"
        doc = source.get("document") or source.get("document_type") or "Unknown document"

        source_str = f"  {i}. {uni} - {doc}"

        # Optional course + section
        if "course" in source:
            section = source.get("section")
            if section:
                source_str += f" - {source['course']} ({section})"
            else:
                source_str += f" - {source['course']}"

        # Optional relevance / score fields
        if "relevance" in source:
            try:
                source_str += f" [Relevance: {source['relevance']:.1%}]"
            except TypeError:
                source_str += f" [Relevance: {source['relevance']}]"
        elif "score" in source:
            try:
                source_str += f" [Score: {source['score']:.3f}]"
            except TypeError:
                source_str += f" [Score: {source['score']}]"
